...
// struct that contains an array of type struct Address
struct Database {
	struct Address rows[MAX_ROWS];
};

// struct contains two pointers: one to a file, one to another struct
struct Connection {
	FILE *file;
	struct Database *db;
};
...
void Database_load(struct Connection *conn)
{
	...
	// successfully read files return 1 
	// a check to see if the file is not corrupted
	if (rc != 1)
...
int main(int argc, char *argv[])
{
	...
	// breaking it by not checking the id is possible
